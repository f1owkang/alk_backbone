...
function show_sitemap(){
	...
	for(var l = 0; l <= l1; l++){
		...
		if (l<= 2) {
		    ...
		    var n = ((l-1) / 4 < 1) ? 0 : 3;
		    $("menu_body").rows[n + 2].cells[(l-1) % 4].innerHTML = map_code;
		}else if(l <= 8) {
			var n = ((l-1) / 4 < 1) ? 0 : 3;
		    $("menu_body").rows[n + 2].cells[(l==8 ? (l-2):(l-1)) % 4].innerHTML = map_code;
		}else {
		    var n = ((l-2) / 4 < 1) ? 0 : 3;
		    $("menu_body").rows[n + 2].cells[(l-2) % 4].innerHTML = map_code;
		}
